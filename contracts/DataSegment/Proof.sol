...
contract Proof {
    ...
    function computeExpectedAuxData(
        ...
        require(isPow2(uint64(_verifierData.sizePc)), "Size of piece provided by verifier is not power of two");
        ...
        SegmentDesc memory en = makeDataSegmentIndexEntry(Fr32(commPc), dataOffset, uint64(_verifierData.sizePc));
    ...
    function validateIndexEntry(InclusionProof memory _ip, uint64 _assumedSizePa) internal pure {
        ...
        (bool ok3, uint64 indexOffset) = checkedMultiply(_ip.proofIndex.index, BYTES_IN_DATA_SEGMENT_ENTRY);
     ...
     */
    function computeRoot(ProofData memory _d, bytes32 _subtree) public pure returns (bytes32) {
     ...
     */
    function verify(ProofData memory _proof, bytes32 _root, bytes32 _leaf) public pure returns (bool) {
     ...
     * @return the root of the merkle tree
     */
    function processProof(ProofData memory _proof, bytes32 _leaf) internal pure returns (bytes32) {
